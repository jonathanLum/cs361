...
def drawgraph():
    ...
    if variables['format'] == "line":
        for i in range(len(variables['entries'])):
            plt.plot(variables['entries'][i][X], variables['entries'][i][Y], color=variables['entries'][i][COLOR] if variables['entries'][i][COLOR] != "" else "k", label=variables['entries'][i][NAME], marker="o")
        ...
        for i in range(len(variables['entries'])):
            if variables['entries'][i][Y] == []:
                continue
            plt.bar(spc + (i * barW), variables['entries'][i][Y], width=barW, color=variables['entries'][i][COLOR] if variables['entries'][i][COLOR] != "" else "lightblue", edgecolor="k", align='edge', label=variables['entries'][i][NAME])
        plt.xticks(spc + ((len(variables['entries'])) * barW) / 2, variables['entries'][0][X])
    elif variables['format'] == "point":
        for i in range(len(variables['entries'])):
            plt.plot(variables['entries'][i][X], variables['entries'][i][Y], ".", color=variables['entries'][i][COLOR] if variables['entries'][i][COLOR] != "" else "k", label=variables['entries'][i][NAME])
